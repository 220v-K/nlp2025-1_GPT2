...
class GPT2Model(GPTPreTrainedModel):
  ...
  def forward(
      ...
      """
      input_ids:      [B, L] 토큰 ID
      attention_mask: [B, L] 1=non-pad, 0=pad
      inputs_embeds:  [B, L, D] 이미 계산된 임베딩 (prefix-tuning 등)
      """
      # 1) 토큰+포지션 임베딩 or 외부 임베딩 사용
      if inputs_embeds is not None:
          embedding_output = inputs_embeds               # [B, L, D]
      else:
          embedding_output = self.embed(input_ids=input_ids)  # [B, L, D]

      # 2) Transformer 블록 스택
      sequence_output = self.encode(embedding_output, attention_mask=attention_mask)  # [B, L, D]
      sequence_output = self.final_layer_norm(sequence_output)                       # [B, L, D]

      # 3) attention_mask가 float일 수 있으니 long으로 캐스팅
      if attention_mask is not None:
          mask = attention_mask.long()
      else:
          # pad가 전혀 없다고 가정
          mask = torch.ones(sequence_output.size(0), sequence_output.size(1),
                            dtype=torch.long, device=sequence_output.device)

      # 4) 마지막 non-pad 토큰 인덱스 계산 및 .long() 보장
      last_non_pad_idx = (mask.sum(dim=1) - 1).long()  # [B]
      ...
      return {
          "last_hidden_state": sequence_output,  # [B, L, D]
          "last_token":         last_token      # [B, D]
      }
