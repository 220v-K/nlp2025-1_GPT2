...
# -*- coding: utf-8 -*-
"""
paraphrase_detection.py로 생성된 체크포인트를 사용하여 dev set에 대해 테스트를 진행하는 스크립트
체크포인트 경로: /home/2020112534/NLP_final/nlp2025-1/checkpoints/250522_default_paraphrase_10-2e-05/10-2e-05-paraphrase.pt

실행:
  python test_250522_default_checkpoint.py --use_gpu
"""
...
def setup_logging():
    ...
    log_filename = f"logs/test_250522_default_{timestamp}.log"
...
# paraphrase_detection.py의 ParaphraseGPT 클래스 재정의
class ParaphraseGPT(nn.Module):
    """Paraphrase Detection을 위해 설계된 GPT-2 Model."""
    ...
    def forward(self, input_ids, attention_mask):
        """Paraphrase detection을 위한 forward pass"""
        # GPT-2 실행
...
def evaluate_checkpoint_on_dev(args):
    ...
    logger.info("=== 250522 Default 체크포인트 Dev Set 평가 시작 ===")
    ...
    try:
        ...
        results_file = f'predictions/250522_dev_evaluation_results_{timestamp}.txt'
        with open(results_file, 'w', encoding='utf-8') as f:
            f.write(f'=== 250522 Default 체크포인트 Dev Set 평가 결과 ===\n')
        ...
        # 성능 요약 파일도 별도로 저장
        summary_file = f'predictions/250522_dev_summary_{timestamp}.txt'
...
def get_args():
    parser = argparse.ArgumentParser(description="paraphrase_detection.py 체크포인트를 사용한 Dev Set 평가")
    
    # 필수 인자들
    parser.add_argument(
        "--checkpoint_path", 
        type=str, 
        default="/home/2020112534/NLP_final/nlp2025-1/checkpoints/250522_default_paraphrase_10-2e-05/10-2e-05-paraphrase.pt",
        help="평가할 체크포인트 경로"
    )
    parser.add_argument("--para_dev", type=str, default="data/quora-dev.csv", help="Dev 데이터 경로")
    parser.add_argument("--para_dev_out", type=str, default="predictions/250522-dev-output.csv", help="Dev 예측 결과 저장 경로")
