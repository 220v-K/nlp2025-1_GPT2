...
def setup_logging():
  ...
  log_file = os.path.join(log_dir, f'paraphrase_detection_{current_time}.log')
...
def check_gpu(logger):
  """GPU 사용 가능 여부와 개수를 확인하고 로깅합니다."""
  if torch.cuda.is_available():
    n_gpu = torch.cuda.device_count()
    logger.info(f"GPU 사용 가능. 총 {n_gpu}개의 GPU 감지.")
...
def save_model(model, optimizer, args, filepath, is_best=False):
  ...
  logging.info(f"모델 저장 완료: {filepath}") # 로깅 추가

  # 최고 성능 모델일 경우 별도 복사
  if is_best:
    best_path = os.path.join('checkpoints', 'best_model.pt')
    torch.save(save_info, best_path)
    logging.info(f"최고 성능 모델 저장 완료: {best_path}") # 로깅 추가
...
def train(args):
  ...
  use_gpu_flag, n_gpu = check_gpu(logger) # GPU 확인 및 로깅
  device = torch.device('cuda') if args.use_gpu and use_gpu_flag else torch.device('cpu')
  ...
  # logger.info(f"훈련 데이터 샘플 수: {len(para_train_data['question1'])}")
  # logger.info(f"개발 데이터 샘플 수: {len(para_dev_data['question1'])}")
  ...
  logger.info(f"모델 초기화 완료. 모델 파라미터 수: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
  ...
  for epoch in range(args.epochs):
    ...
    for batch_idx, batch in enumerate(tqdm(para_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
      ...
      if global_step % log_interval == 0 and batch_count > 0:
        ...
        logger.info(f"Epoch {epoch}, Step {global_step}: train loss {avg_batch_loss:.3f}, train acc {avg_batch_acc:.3f}") # 로깅 추가
    ...
    if is_best:
      ...
      logger.info(f"Epoch {epoch}: 새로운 최고 검증 정확도 달성! Acc: {dev_acc:.3f}, F1: {dev_f1:.3f}") # 로깅 추가

    print(f"Epoch {epoch} 완료: train loss :: {train_loss :.3f}, train acc :: {train_acc :.3f}, dev acc :: {dev_acc :.3f}, dev f1 :: {dev_f1 :.3f}")
    logger.info(f"Epoch {epoch} 완료: train_loss={train_loss:.3f}, train_acc={train_acc:.3f}, dev_acc={dev_acc:.3f}, dev_f1={dev_f1:.3f}") # 로깅 추가
  ...
  if args.use_gpu and use_gpu_flag: # GPU 사용 여부 확인
    logger.info(f"훈련 중 최대 메모리 사용량: {peak_memory_allocated:.2f} MB")
...
def test(args):
  ...
  if os.path.exists(best_model_path):
    ...
    logger.info(f"최고 성능 모델을 불러옵니다: {checkpoint_to_load}")
  else:
    checkpoint_path = os.path.join('checkpoints', args.filepath)
    if os.path.exists(checkpoint_path):
      checkpoint_to_load = checkpoint_path
      logger.info(f"체크포인트를 불러옵니다: {checkpoint_to_load}")
    elif os.path.exists(args.filepath): # args.filepath가 파일 경로일 경우
      checkpoint_to_load = args.filepath
      logger.info(f"기존 모델을 불러옵니다: {checkpoint_to_load}")
    else:
      logger.error("로드할 모델 파일이 없습니다. 테스트를 중단합니다.")
      return

  saved = torch.load(checkpoint_to_load, map_location=device) # map_location 추가

  model_args = saved.get('args', args) # args가 없을 경우 현재 args 사용
  model = ParaphraseGPT(model_args)
  model.load_state_dict(saved['model'])
  model = model.to(device)
  model.eval()
  logger.info(f"모델 로드 완료: {checkpoint_to_load}")

  logger.info("테스트 데이터 로드 시작...")
  para_dev_data = load_paraphrase_data(args.para_dev)
  para_test_data = load_paraphrase_data(args.para_test, split='test')
  logger.info(f"개발 데이터 샘플 수: {len(para_dev_data['question1'])}")
  logger.info(f"테스트 데이터 샘플 수: {len(para_test_data['question1'])}")
  ...
  logger.info(f"dev paraphrase acc: {dev_para_acc:.3f}, dev f1: {dev_para_f1:.3f}") # 로깅 추가
  test_para_y_pred, test_para_sent_ids = model_test_paraphrase(para_test_dataloader, model, device)
  logger.info("테스트 세트 예측 완료.")
  ...
  logger.info(f"개발 세트 예측 결과 저장 완료: {args.para_dev_out}")
  logger.info(f"테스트 세트 예측 결과 저장 완료: {args.para_test_out}")
  logger.info("테스트 완료.")
...
def get_args():
  ...
  parser.add_argument("--batch_size", help='sst: 64, cfimdb: 8 can fit a 12GB GPU', type=int, default=96)
...
if __name__ == "__main__":
  ...
  args = add_arguments(args) # 모델 크기에 따른 인수 추가
  seed_everything(args.seed)  # 재현성을 위한 random seed 고정.
  logger.info(f"Random seed 고정: {args.seed}")
