...
class CausalSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    bs, heads, seq_len, _ = query.size()

    # Step 1: Scaled dot-product attention
    attention_scores = torch.matmul(query, key.transpose(-1, -2))  # [bs, heads, seq, seq]
    attention_scores = attention_scores / math.sqrt(self.attention_head_size)

    # Step 2: Add causal mask (lower triangular)
    ...
    # Step 3: Add attention mask (for padding)
    if attention_mask is not None:
        attention_scores = attention_scores + attention_mask  # [bs, 1, 1, seq]

    # Step 4: Normalize and apply dropout
    attention_probs = torch.nn.functional.softmax(attention_scores, dim=-1)
    attention_probs = self.dropout(attention_probs)

    # Step 5: Weighted sum of values
    context_layer = torch.matmul(attention_probs, value)  # [bs, heads, seq, head_dim]
    context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # [bs, seq, heads, head_dim]
    context_layer = rearrange(context_layer, 'b t h d -> b t (h d)')  # [bs, seq, all_head_size]
